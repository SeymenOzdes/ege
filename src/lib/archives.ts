...
export const getLatestArticles = cache(async (page: number): Promise<ArchivePage | undefined> =>
...
/**
 * The taxonomy changes on the order of once a month, and `/kategori/[slug]` is a
 * dynamic route — it reads `sayfa` from the query string — so nothing above it
 * caches these two reads. An hour is far shorter than the tables actually move.
 */
const FACETS_REVALIDATE_SECONDS = 3600;

/** Invalidation hook for whenever topics or cities become editable in the admin. */
export const ARCHIVE_FACETS_TAG = "archive-facets";

/**
 * Topic and city lists for the category page's wayfinding rail.
 *
 * `getSearchFacets` answers the same question but runs on the cookie-bound server
 * client, which opts the caller out of caching. A category archive is public and
 * anonymous, so it reads through `createAnonClient` instead, which is what makes the
 * result cacheable across requests rather than merely deduplicated within one.
 *
 * Both layers earn their place: `unstable_cache` keeps the rail off the database
 * between revalidations, and `cache()` collapses the repeat calls a single render
 * makes. (`unstable_cache` rather than `use cache`, which the Next 16 docs prefer —
 * that directive needs Cache Components enabled project-wide, which is a migration
 * of its own and not this module's to make.)
 */
const readArchiveFacets = unstable_cache(
  async (): Promise<SearchFacets> => {
    if (!hasSupabasePublicConfig()) return emptyFacets;

    return readFacets(createAnonClient());
  },
  [ARCHIVE_FACETS_TAG],
  { revalidate: FACETS_REVALIDATE_SECONDS, tags: [ARCHIVE_FACETS_TAG] },
);

export const getArchiveFacets = cache(readArchiveFacets);
